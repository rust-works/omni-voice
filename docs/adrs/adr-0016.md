...
> Amended (2026-06-17): the command tree below describes the pre-refactor toolkit (the `git`, `ai`, `commands`, and `config` domains) and is retained as historical context. The strip-to-voice-cli refactor first reduced the surface to a `voice` command group; a follow-up flattened that group so its leaves are now top-level `Commands` variants — `capture`, `transcribe`, `reflect`, `review`, `install-model`, and `enroll`, alongside the hidden `completions` command and `help-all`. The tree is one level deep, with `reflect` the sole async command. The clap-derive decision below still governs that surface.
