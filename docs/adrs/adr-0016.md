...
> Amended (2026-06-16): the command tree below describes the pre-refactor toolkit (the `git`, `ai`, `commands`, and `config` domains) and is retained as historical context. The strip-to-voice-cli refactor reduced the surface to three top-level `Commands` variants — `voice` (with `capture`/`transcribe`/`reflect`/`review`/`install-model`/`enroll` leaves), the hidden `completions` command, and `help-all` — at most two levels deep, with `voice reflect` the sole async leaf. The clap-derive + hierarchical-subcommand decision below still governs that surface.
