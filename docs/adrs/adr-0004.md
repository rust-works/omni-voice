...
> Amended (2026-06-16): only `models.yaml` (`claude::model_config`) remains embedded. The `default-commit-guidelines.md` prompt template (`claude::prompts`) and the `cli::config` model-display module were removed in the strip-to-voice-cli refactor; the three scaffold command templates (`commit-twiddle.md`, `pr-create.md`, `pr-update.md`) and their `cli::commands` module were removed when the top-level `commands` command was dropped. The `include_str!` decision below still governs `models.yaml`; the multi-template inventory and usage categories are retained as historical context.
...
- **Commit guidelines have a runtime override path.** `claude::prompts` only falls back to
  ...
  No other template type currently has an equivalent
