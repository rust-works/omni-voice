...
> No longer applies: the preflight validation module (`src/utils/preflight.rs`) and the git/AI commands it guarded (`twiddle`, `check`, `create-pr`, `amend`, `view`, `info`, `chat`) were removed in the strip-to-voice-cli refactor (issue [#36](https://github.com/rust-works/omni-voice/issues/36)). Retained as a historical record of the original rationale.
