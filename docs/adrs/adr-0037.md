...
Originally: ✅ Accepted (2026-06-06) — relaxes [ADR-0033](adr-0033.md); extends
[ADR-0035](adr-0035.md).

[ADR-0041](adr-0041.md) narrows omni-voice to Apple Silicon macOS, so the
"non-Windows targets" framing that defines this ADR collapses to "Apple Silicon"
— and [ADR-0042](adr-0042.md) chooses a *subprocess* MLX backend over the
in-process pure-C FFI path for v1. The voxtral.c native-C+Metal FFI option
described here survives only as a *future* Python-free candidate, re-scoped under
ADR-0042 (and gated on relaxing `unsafe_code = "deny"`).
