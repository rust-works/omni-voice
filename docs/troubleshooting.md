...
environment variable. Re-download with `install-model --force`.
...
WAV**. Produce a compliant file with `capture` (which records at 16 kHz
...
`capture` matches `--device` exactly against the platform-reported input
...
```bash
omni-voice capture --device "MacBook Pro Microphone"
```
...
threshold or re-enroll in a quieter environment with `enroll`.

## Reflection (`reflect`)

`reflect` is the only command that calls an AI model, so it needs a
...
`review <SESSION_ID>` reads `~/.omni-voice/voice/<id>/`. Pass the session
...
- **Model-gated tests skipped**: some voice tests require installed models and
  are ignored when the models are absent — run `install-model` first.
