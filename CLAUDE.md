...
3. **Configuration Changes**: When modifying config loading (e.g. `models.yaml` resolution or `src/utils/settings.rs`), consult [docs/plan/config-internals.md](docs/plan/config-internals.md) and the [`.omni-voice/` directory contract](docs/omni-voice-directory.md)
