...
### Added
- **`omni-voice listen` — realtime capture→transcribe→reflect scheduler** ([#8](https://github.com/rust-works/omni-voice/issues/8)): A new top-level `omni-voice listen --session <id>` command that runs the live pipeline continuously — it captures the microphone, streams it through the streaming ASR seam (`StreamingTranscriber`, [ADR-0038](docs/adrs/adr-0038.md)), and fires `reflect` on a trigger heuristic as the transcript arrives, persisting to `~/.omni-voice/voice/<id>/`. A dedicated-thread **cpal supervisor** (`src/voice/listen/supervisor.rs`) mixes down/resamples/quantises capture to 16 kHz mono i16 and re-opens the stream on device failure behind an exponential backoff (100 ms→5 s, give up after 20 retries), feeding a bounded `tokio::mpsc` bridge (`--audio-buffer-size`, default 1600 frames = 100 ms; queue capacity 160 ≈ 16 s) that drops newest-on-full and never blocks the audio thread. The **reflection scheduler** (`src/voice/listen/scheduler.rs`) reflects on **any** of a silence gap (`--trigger-silence-gap-ms`, default 1500), a finalized-word delta (`--trigger-word-delta`, default 30), or a max-interval floor (`--trigger-max-interval-ms`, default 60000), all gated by a non-configurable 3 s min-interval; only one reflection runs at a time (bursts collapse into a single follow-up), each reusing `reflect`'s session-mode `run_reflect` so `events.jsonl`, `meta.last_reflected_event_id`, and the per-reflection `reflections.log` line are written exactly as `reflect` does. Auto-stops after `--idle-after` seconds of silence or on Ctrl-C, draining one final reflection over any un-reflected transcript. A new streaming factory (`create_default_streaming_transcriber`) wires the streaming-capable backends — `voxtral-mlx` (the default) and a new dependency-free `mock` streaming backend (`MockStreamingTranscriber`) for CI — and **rejects** the batch-only backends (`whisper-candle`, `whisper-candle-streaming`, `parakeet-tdt`) with a fail-fast message *before* the microphone is opened. Covered by a model-free end-to-end smoke test (mock streaming backend + fixture on a clock → ≥3 `item.create` events, with transcript/reflection-log assertions) plus unit tests for the pure trigger core, the backoff, the mixdown/resample bridge, and the idle/shutdown paths; the `help-all` snapshot is regenerated. Per-reflection **cost** in `reflections.log` is tracked separately in [#64](https://github.com/rust-works/omni-voice/issues/64) (latency is already recorded); live `--speaker` filtering ships separately in [#5](https://github.com/rust-works/omni-voice/issues/5) (see below). A hidden `--audio-file <PATH>` flag replays a 16 kHz mono WAV at realtime pace instead of opening the microphone, for reproducible end-to-end runs and demos without audio hardware. New modules under `src/voice/listen/` and `src/cli/voice/listen.rs`.
- **Live speaker locking for `omni-voice listen` (`--speaker`)** ([#5](https://github.com/rust-works/omni-voice/issues/5)): `omni-voice listen --speaker <name>` transcribes **only** the enrolled speaker — other voices in the room are dropped — turning the listen loop into a project-aware "voice second brain" you can run with people around. It reuses the exact wespeaker cosine gate batch `transcribe --speaker` ships ([ADR-0034](docs/adrs/adr-0034.md)): because the streaming event seam is text-only, a new consumer-side **`TeeAudioInput`** mirrors each 16 kHz chunk into a bounded rolling **`PcmRing`** (30 s) *after* the capture channel's drop-on-overflow point, so the ring stays sample-aligned with what the backend actually consumed; on each `Final` a `SpeakerGate` slices the ring by the event's `start`/`end` timestamps, embeds the slice (`WespeakerEmbedder`), and keeps the segment only when its cosine similarity to the enrolled embedding clears `--speaker-threshold` (default 0.5, shared with `transcribe --speaker` via the relocated `DEFAULT_SPEAKER_THRESHOLD`). The gate runs **before** transcript-append and the reflection trigger, so rejected speech never reaches `transcript.jsonl` or fires a reflection; kept `Final`s are stamped with the speaker name. Gating is **per-segment (per `Final`)**, not per-frame. It fails **fast** at startup (before the microphone opens) on a missing enrolment — pointing at `omni-voice enroll` — or a missing model, and fails **open** at runtime (keeps + warns) if a segment's audio has already scrolled out of the ring, an embedding errors, or a dimensionality mismatch appears, so a gate hiccup never silently swallows your own speech; a too-short segment (< 0.5 s) is dropped as batch does. New flags `--speaker`, `--speaker-threshold`, `--speaker-model` on `listen`; disabling (`--speaker` unset) is zero-overhead (no ring, no tee). Covered by `PcmRing`/tee unit tests and a model-gated accept-enrolled / reject-other end-to-end test (`tests/voice_listen_speaker_gate_test.rs`, `#[ignore]` pending the CI model stage in [#13](https://github.com/rust-works/omni-voice/issues/13)); the `help-all` snapshot is regenerated. New module `src/voice/listen/speaker_gate.rs`. (Assistant authority scoping and per-reflection cost remain tracked in [#10](https://github.com/rust-works/omni-voice/issues/10) / [#9](https://github.com/rust-works/omni-voice/issues/9) / [#11](https://github.com/rust-works/omni-voice/issues/11) / [#64](https://github.com/rust-works/omni-voice/issues/64).)
- **Nearest-speaker labelling for `omni-voice listen` (`--label`, repeatable `--speaker`)** ([#69](https://github.com/rust-works/omni-voice/issues/69)): Generalises the [#5](https://github.com/rust-works/omni-voice/issues/5) single-speaker gate from a binary keep/drop into an **N-way labeller** that answers *"who said what"* — with two or more speakers enrolled, each `Final` is embedded once, cosined against **every** enrolled speaker, and tagged in `transcript.jsonl` with the *nearest* one above `--speaker-threshold`. `--speaker` is now **repeatable**: a single `--speaker <name>` keeps the #5 gate unchanged (drop others), while `--speaker a --speaker b` labels that subset; the new `--label` flag labels against **all** enrolled speakers (mutually exclusive with `--speaker`). A segment matching nobody above the threshold (or too short to embed) follows the new `--unknown-policy <keep|drop>` (default `keep` → tagged `speaker: "unknown"` and still transcribed; `drop` preserves the gate's behaviour). The `SpeakerGate` now holds a `Vec<EnrolledSpeaker>` and a `decide()` method returning keep-with-tag / drop, sharing the same `PcmRing` / `TeeAudioInput` / `WespeakerEmbedder` / fail-open scaffolding as #5 (label-mode fail-open leaves the segment unattributed rather than misattributing it; enrolments whose embedding dimension doesn't match are skipped rather than panicking). A new `load_all_enrolled()` enumerates every `~/.omni-voice/voice/speakers/*.json` for `--label`. Covered by model-free `decide()` unit tests (nearest-of-N arg-max, unknown-policy keep/drop, too-short, fail-open, dim-mismatch skip), CLI parse tests (repeatable `--speaker`, `--label`/`--unknown-policy`, the `--label`↔`--speaker` conflict), and a model-gated end-to-end test enrolling two speakers off `two_speakers.wav` and asserting speaker-A windows label `A` and speaker-B windows label `B` (`tests/voice_listen_speaker_gate_test.rs`, `#[ignore]` pending the CI model stage in [#13](https://github.com/rust-works/omni-voice/issues/13)); the `help-all` snapshot is regenerated. Turn-level segmentation and overlapping speech remain out of scope (diarization proper / target-speaker extraction). Extends `src/voice/listen/speaker_gate.rs`, `src/voice/speaker.rs`, and `src/cli/voice/listen.rs`.
- **`install-model` download confirmation prompt** ([#14](https://github.com/rust-works/omni-voice/issues/14)): `omni-voice install-model` now prints the source URL and expected size and prompts `Proceed? [y/N]` on stdin before any bytes are transferred, instead of downloading silently. The prompt fires **only** when a download will actually happen — the idempotent "model already installed" skip path is unchanged and never prompts. A new `--accept-downloads` flag (or `OMNI_VOICE_AUTO_DOWNLOAD=true`) bypasses it for non-interactive automation; on a non-interactive stdin without either bypass, the command fails loudly pointing at the flag/env var rather than blocking forever on `read_line`. The confirmation summary is driven by a new `ModelSpec::download_summary()` (source URL + human-readable size). The `install-model` `--help` snapshot is regenerated.
- **HfHub model-download integrity verification** ([#29](https://github.com/rust-works/omni-voice/issues/29)): `install-model` now verifies the integrity of **every** file fetched from HuggingFace Hub before installing it, closing the gap where a corrupt, truncated, or interrupted-and-resumed download installed silently and only surfaced later as an opaque load failure or subtly-wrong weights (the release-asset path already verified SHA-256). Each cached file is checked against the content identity hf-hub stored it under — the SHA-256 LFS OID for large weight files, the git blob SHA-1 for small non-LFS files (`config.json`, `tekken.json`, …) — and bails on mismatch, mirroring `download_release_asset`. No hashes are hardcoded, so verification tracks any revision automatically; hashes stream in 64 KiB chunks so multi-GB weights are never read into memory. Covers `whisper-tiny.en`, `voxtral-mlx-int4`, and the Parakeet upstream download; a bad download re-installs cleanly on re-run via the existing atomic `.part`+rename. New dependency: `sha1`.
- **`whisper-candle-streaming` ASR backend** ([#974](https://github.com/rust-works/omni-voice/issues/974)): A pure-Rust **streaming** Whisper backend (`omni-voice transcribe --backend whisper-candle-streaming`) — the latency-tolerant, pure-Rust streaming baseline (the low-latency headline tier is the planned streaming-native MLX backend, [ADR-0042](docs/adrs/adr-0042.md)). It ports the algorithm validated by the [#969](https://github.com/rust-works/omni-voice/issues/969) spike onto the synchronous `Transcriber` seam: an `earshot` voice-activity gate (new `src/voice/vad.rs`) silence-gates the decode window so the decoder never transcribes silence, the window is re-decoded on a cadence (skipped entirely when unchanged — the redundant-inference skip), LocalAgreement-2 commits the stable word prefix as non-revisable `Final`s while the volatile tail streams as `Partial`s, and a hard window cap re-decodes the full window before flushing (lossless cap). `Final`s carry real average-logprob confidence from the shared `WhisperEngine` (extracted from `src/voice/backends/candle.rs`; the batch backend reuses the identical inference). Defaults are the #969-validated operating envelope (cap 5 s, cadence 1.0 s, silence 0.3 s, min-window 2.0 s, VAD 0.5 — RTF 0.34 / WER 9.2 % / time-to-final 0.73–1.42 s on the reference run); knobs are exposed on the Rust API via `StreamingConfig`. **Latency caveat**: the transcript trails the speaker by a bounded ~1.5–3 s — sub-second interactive latency is structurally out of scope for this backend (decision, root cause, tiering vs Voxtral, and rejected approaches recorded in [ADR-0040](docs/adrs/adr-0040.md)). Covered by model-free unit tests over a scripted decoder seam, model-gated envelope tests (`tests/voice_streaming_candle_test.rs`: WER/RTF/determinism gates plus a deadline-paced 1× driver gating time-to-final ≤ 2.5 s and bounded non-drifting display lag). New backend doc: [docs/asr-backends.md](docs/asr-backends.md).
...
### Changed
- **Rescoped to Apple Silicon macOS only** (BREAKING) ([#42](https://github.com/rust-works/omni-voice/issues/42)): omni-voice now supports **only** Apple Silicon macOS (`aarch64-apple-darwin`), dropping the cross-platform / pure-Rust / no-C++ / Windows constraints that forced the latency-tolerant candle streaming floor. A `compile_error!` build guard in `src/lib.rs` fails the build fast on any other target. CI build/test jobs run on `macos-latest` (ALSA install steps removed; the Docker-based dependency-policy and secret-scanning jobs stay on Linux); release builds only the `aarch64-apple-darwin` target (the Linux and Windows matrix arms and the Windows-zip branch are dropped, and `cargo publish` moves to a macOS runner); the cross-platform `voice-streaming-keepup` workflow is removed; `flake.nix` narrows to `aarch64-darwin`; and `nix` becomes an unconditional dependency. The decision and its consequences are recorded in **[ADR-0041](docs/adrs/adr-0041.md)** (platform commitment) and **[ADR-0042](docs/adrs/adr-0042.md)** (native Apple-Silicon ASR backend selection — a streaming-native MLX subprocess as the planned low-latency tier, `whisper-candle-streaming` demoted to the pure-Rust baseline, and the candle backends moving to the Metal GPU). ADR-0035/0037/0040 are superseded and ADR-0033/0034 amended accordingly.
- **Voice commands promoted to the top level** (BREAKING): the `voice` command group has been removed and its six subcommands — `capture`, `transcribe`, `reflect`, `review`, `install-model`, and `enroll` — are now top-level commands. Invocations drop the `voice` segment: `omni-voice voice transcribe recording.wav` becomes `omni-voice transcribe recording.wav` (and likewise for the others). The `VoiceCommand` / `VoiceSubcommands` wrapper types are gone; each command struct is now wired directly into the top-level `Commands` enum in `src/cli.rs`. Shell completions, the `help-all` output, and all install/error hints have been regenerated accordingly.

### Removed
- **ADF schema codegen and drift-detection tooling** ([#731](https://github.com/rust-works/omni-voice/issues/731), [#732](https://github.com/rust-works/omni-voice/issues/732)): Removed the `adf-schema-codegen` and `adf-schema-drift` binaries (`src/bin/`), the `drift` and `generated` modules under `src/atlassian/adf_schema/`, the vendored upstream schema assets (`assets/adf-schema/`), the weekly `adf-schema-drift.yml` GitHub Actions workflow, and the `adf-schema-drift` binary integration tests. The hand-maintained `CONTENT_ENTRIES` allowed-children table in `src/atlassian/adf_schema/mod.rs` becomes the sole source of truth; the runtime validator (`validate_document`) and its provenance constants (`SCHEMA_VERSION`, `UPSTREAM_TARBALL_SHA256`) are unchanged. The "Refreshing the vendored schema" and drift-detector sections of the contributing guide are pruned accordingly; refresh against upstream `@atlaskit/adf-schema` is now a manual table edit.
